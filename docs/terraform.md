...
| <a name="input_dd_fetch_lambda_tags_list"></a> [dd\_fetch\_lambda\_tags\_list](#input\_dd\_fetch\_lambda\_tags\_list) | List of sepcific ARNs the Lambda is allowed to collect tags from, defaults to `["arn:aws:lambda:::*"]`. | `list(string)` | <pre>[<br>  "arn:aws:lambda:::*"<br>]</pre> | no |
